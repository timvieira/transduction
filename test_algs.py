...
def _run_logp_test(fst, max_source_len=None, target_depth=None, seed=42, atol=1e-8):
    ...
    def check(target):
        ...
        got_v5 = inc.logp_next_v5(target)
        ...
        for y in target_alphabet:
            ...
            got_v5_val = got_v5[y] if y in got_v5 else float('-inf')
            ...
            assert got_v5_val == pytest.approx(ref_val, abs=atol), \
                f"target={target} symbol={y}: got_v5={got_v5_val} ref={ref_val}"
        ...
        got_v5_eos = got_v5[inc.EOS] if inc.EOS in got_v5 else float('-inf')
        ...
        assert got_v5_eos == pytest.approx(ref_eos, abs=atol), \
            f"target={target} EOS: got_v5={got_v5_eos} ref={ref_eos}"
