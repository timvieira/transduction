...
class RustPeekabooState:
    ...
    _LAZY_BFS_ATTRS = frozenset({
        'decomp', 'dfa', 'resume_frontiers', 'preimage_stops', 'generation',
    })
    ...
    def _ensure_bfs(self):
        ...
        self.dfa = dfa
        self.generation = rust_decomp.generation()
        self._bfs_done = True
...
# ---------------------------------------------------------------------------
# RustPositionSetPeekabooState: position-set peekaboo for TD FSTs
# ---------------------------------------------------------------------------

class _RustPositionSetPeekabooUniv:
    """No-op universality placeholder — Rust handles universality internally."""
    def __init__(self, fst):
        pass


class RustPositionSetPeekabooState:
    """Position-set peekaboo backed by Rust, for token-decomposable FSTs.

    Same interface as RustPeekabooState but uses PositionSetPeekaboo under
    the hood, giving dramatic state compression for TD FSTs (e.g. 174x:
    ...
    def __init__(self, fst, target=(), parent=None, *, univ=None,
        ...
        if _rust_state is not None:
            ...
            import transduction_core
            rust_fst, sym_map, state_map = to_rust_fst(fst)
            rust_decomp = transduction_core.RustPositionSetPeekabooDecomp(rust_fst)
            self._rust_state = (rust_decomp, sym_map, state_map)

        self._bfs_done = False

    def __getattr__(self, name):
        if name in RustPositionSetPeekabooState._LAZY_BFS_ATTRS:
            self._ensure_bfs()
            return self.__dict__[name]
        raise AttributeError(
            f"'{type(self).__name__}' object has no attribute {name!r}"
    ...
    def decompose_next(self):
        """Returns {y: RustPositionSetPeekabooState} for all next target symbols."""
    ...
    def quotient(self):
        ...
        if '_qr_cache' not in self.__dict__:
            self._qr_cache = self._parent.build_qr_fsa(self.target[-1])
    ...
    def __rshift__(self, y):
        return RustPositionSetPeekabooState(
            self.fst, self.target + (y,),
            parent=self,
            _rust_state=self._rust_state,
        )
