...
| `TargetSideBuffer` | unconditional accumulation | none (unbounded) | `(i, ys)` | IncrementalDFADecomp (via Relevance) |
| `PeekabooLookaheadNFA` | push, K-lookahead | at N+K with truncation bit | `(i, ys, truncated)` | PeekabooState |
| `PeekabooFixedNFA` | push, fixed N+1 | at N+1, no truncation bit | `(i, ys)` | Peekaboo (nonrecursive), RustPeekaboo |
