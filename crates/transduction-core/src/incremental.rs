...
impl DirtyDecomp {
    ...
    pub fn materialize_quotient(&mut self) -> FsaResult {
        ...
        let (num_states, arc_src, arc_lbl, arc_dst) = self.collect_arcs();
        FsaResult {
            num_states,
            start: vec![self.last_start],
            stop: self.last_q_stop.clone(),
            arc_src,
            arc_lbl,
            arc_dst,
        }
    ...
    /// Collect arc arrays from cached arcs_from for all reachable non-QSTOP states.
    fn collect_arcs(&self) -> (u32, Vec<u32>, Vec<u32>, Vec<u32>) {
        let num_states = self.arena.len() as u32;
        let mut src = Vec::new();
        let mut lbl = Vec::new();
        let mut dst = Vec::new();
        for &sid in &self.reachable {
            if self.state_status[sid as usize] != STATUS_QSTOP {
                for &(l, d) in &self.arcs_from[sid as usize] {
                    src.push(sid);
                    lbl.push(l);
                    dst.push(d);
                }
